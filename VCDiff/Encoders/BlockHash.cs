...
namespace VCDiff.Encoders
{
    public class BlockHash
    {
        ...
        BigArray<long> hashTable;
        BigArray<long> nextBlockTable;
        BigArray<long> lastBlockTable;
        long tableSize = 0;
        RollingHash hasher;
        ...
        public BlockHash(IByteBuffer sin, int offset, RollingHash hasher)
        {
            ...
            hashTable = new BigArray<long>((ulong)tableSize);
            nextBlockTable = new BigArray<long>((ulong)BlocksCount);
            lastBlockTable = new BigArray<long>((ulong)BlocksCount);
            lastBlockAdded = -1;
            SetTablesToInvalid();
        }
        ...
        {
            for(ulong i = 0; i < nextBlockTable.Length; i++)
            {
                lastBlockTable[i] = -1;
                nextBlockTable[i] = -1;
            }
            for(ulong i = 0; i < hashTable.Length; i++)
            {
                hashTable[i] = -1;
            }
        }
        ...
        public void AddBlock(ulong hash)
        {
            ...
            if(nextBlockTable[(ulong)blockNumber] != -1)
            {
                return;
            }

            long tableIndex = GetTableIndex(hash);
            long firstMatching = hashTable[(ulong)tableIndex];
            if(firstMatching < 0)
            {
                hashTable[(ulong)tableIndex] = blockNumber;
                lastBlockTable[(ulong)blockNumber] = blockNumber;
            }
            else
            {
                long lastMatching = lastBlockTable[(ulong)firstMatching];
                if(nextBlockTable[(ulong)lastMatching] != -1)
                {
                    return;
                }
                nextBlockTable[(ulong)lastMatching] = blockNumber;
                lastBlockTable[(ulong)firstMatching] = blockNumber;
            }
        ...
        {
            return SkipNonMatchingBlocks(hashTable[(ulong)GetTableIndex(hash)], toffset, target);
        }
        ...
        public long NextMatchingBlock(long blockNumber, long toffset, IByteBuffer target)
        {
            ...
            return SkipNonMatchingBlocks(nextBlockTable[(ulong)blockNumber], toffset, target);
        }
        ...
        public long SkipNonMatchingBlocks(long blockNumber, long toffset, IByteBuffer target)
        {
            ...
            while ((blockNumber >= 0) && !BlockContentsMatch(blockNumber, toffset, target))
            {
                ...
                blockNumber = nextBlockTable[(ulong)blockNumber];
            }
            return blockNumber;
        }
